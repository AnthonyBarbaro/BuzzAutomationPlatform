...
REPORT_URL = "https://dusk.backoffice.dutchie.com/reports/marketing/reports/loyalty-adjustment-report"
REPORT_NAME = "Loyalty Adjustment Report"
...
def _store_code_for_name(store_name: str) -> str:
    return sales_report.store_abbr_map.get(store_name, re.sub(r"[^A-Z0-9]+", "", store_name.upper())[:8] or "STORE")
...
def loyalty_adjustment_filename(
    current_store: str,
    start_date: datetime,
    end_date: datetime,
    extension: str = ".xlsx",
    ...
    return _safe_filename(
        f"{store_code} - Loyalty Adjustment Report - {nice} - "
...
def open_loyalty_report_page(driver, timeout: int = REPORT_READY_TIMEOUT_SECONDS) -> bool:
    if "loyalty-adjustment-report" not in str(driver.current_url):
        driver.get(REPORT_URL)
    ...
    try:
        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located(
                (By.XPATH, "//*[normalize-space()='Loyalty Adjustment Report']")
            )
        )
        cells = driver.find_elements(By.XPATH, "//*[normalize-space()='Loyalty Adjustment Report']")
        target = None
        for cell in cells:
            try:
...
def ensure_logged_in(driver, timeout: int = 90) -> bool:
    ...
    while time.time() < end:
        ...
        try:
            ...
            print(f"[LOYALTY EXPORT] Login/page readiness wait is still settling: {exc}")

        time.sleep(1.0)

    return open_loyalty_report_page(driver, timeout=10)
...
def _visible_date_inputs(driver) -> list:
    ...
    for elem in candidates:
        try:
            ...
            if (
                "auth_input" in data_testid
                or placeholder in {"username", "password"}
...
def wait_for_date_inputs(driver, timeout: int = DATE_INPUT_TIMEOUT_SECONDS) -> list:
    ...
    while time.time() < end:
        ...
        time.sleep(0.5)
    raise TimeoutException(f"Could not find two date inputs on loyalty adjustment report. Current URL: {last_url}")


def wait_for_report_page_ready(driver, timeout: int = REPORT_READY_TIMEOUT_SECONDS) -> bool:
    wait = WebDriverWait(driver, timeout)
    try:
        wait.until(lambda d: "loyalty-adjustment-report" in str(d.current_url))
...
def select_store(driver, store_name: str) -> bool:
    ...
    for attempt in range(1, 4):
        try:
            if not open_loyalty_report_page(driver):
                raise TimeoutException("Loyalty adjustment report page did not become ready.")
...
def set_loyalty_date_range(driver, start_date: datetime, end_date: datetime) -> None:
    if not open_loyalty_report_page(driver):
        raise TimeoutException("Loyalty adjustment report page did not become ready before setting dates.")
...
def click_actions_and_export(current_store: str, start_date: datetime, end_date: datetime, output_dir: str | os.PathLike[str]) -> Optional[Path]:
    try:
        print(f"\n=== Exporting loyalty adjustment report for store: {current_store} ===")
        ...
        if table_has_no_data(sales_report.driver):
            print(f"[LOYALTY EXPORT] No loyalty adjustment rows visible for {current_store}.")
        ...
        if not exported_path:
            print(f"No loyalty adjustment export detected within {sales_report.EXPORT_DOWNLOAD_TIMEOUT_SECONDS}s.")
            return None

        extension = os.path.splitext(exported_path)[1] or ".xlsx"
        new_filename = loyalty_adjustment_filename(current_store, start_date, end_date, extension=extension)
        new_path = Path(files_dir) / new_filename
        if new_path.exists():
            new_path.unlink()
        os.rename(exported_path, new_path)
        if new_path.stat().st_size <= 0:
            print(f"[WARN] Downloaded loyalty adjustment file is empty: {new_path}")
            return None

        print(f"Saved loyalty adjustment report: {new_filename}")
        return new_path

    except TimeoutException:
        print("A loyalty adjustment report element could not be found or clicked within the timeout period.")
        return None
    except Exception:
        print(f"An error occurred during loyalty adjustment export: {traceback.format_exc()}")
        return None


def _click_actions_and_export_with_driver(
    driver,
    current_store: str,
    start_date: datetime,
    end_date: datetime,
    ...
    sales_report.driver = driver
    try:
        return click_actions_and_export(current_store, start_date, end_date, output_dir)
    finally:
        sales_report.driver = previous_driver
...
def export_store_with_retries(
    ...
    for attempt in range(1, attempts + 1):
        print(f"[LOYALTY EXPORT] {current_store}: attempt {attempt}/{attempts}")
        try:
            if not select_store(sales_report.driver, current_store):
                raise TimeoutException(f"Could not select store '{current_store}' on loyalty adjustment report.")
            set_loyalty_date_range(sales_report.driver, start_date, end_date)
            ...
            print(f"[WARN] Loyalty adjustment export attempt {attempt} failed for {current_store}: {traceback.format_exc()}")
        time.sleep(3)
    return None
...
def _run_single_store_browser_export(
        ...
        try:
            ...
            try:
                if not ensure_logged_in(driver):
                    last_error = (
                        f"{store}: loyalty adjustment report page did not become ready "
    ...
    return store_code, None, last_error or f"{store}: loyalty adjustment browser session failed"


def run_loyalty_adjustment_report(
    start_date: datetime,
    end_date: datetime,
    ...
    worker_count = max(1, min(int(workers or 1), len(selected_stores) or 1, MAX_LOYALTY_BROWSER_WORKERS))

    exported_paths: Dict[str, Path] = {}
    failures: list[str] = []

    if worker_count > 1:
        print(f"[LOYALTY EXPORT] Using {worker_count} browser workers.")
        ...
        try:
            if not ensure_logged_in(driver):
                failures.append("Loyalty adjustment report page did not become ready after login.")
            else:
                for store in selected_stores:
                    try:
    ...
    if failures:
        message = "Loyalty adjustment export failed for: " + "; ".join(failures)
        if fail_on_error:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download the Dutchie Backoffice loyalty adjustment report.")
    ...
    parser.add_argument(
        ...
        help=f"Parallel browser sessions. Default: {DEFAULT_LOYALTY_BROWSER_WORKERS}; max: {MAX_LOYALTY_BROWSER_WORKERS}.",
    )
...
def main():
    ...
    print(f"[LOYALTY EXPORT] Exported {len(exported)} file(s).")
